...
function page_css(): string
{
    return <<<'CSS'
    :root {
        --text-main: #253247; --text-main-alt: #FAFAFA; --text-primary: #18212F;
        --text-secondary: #5F728F; --text-tertiary: #8190A8; --text-accent: #3D5376;
        --text-error: #B35D40;
        --background-page: #FAFAFA; --background-section-light: #F0F3F7;
        --background-section-dark: #E6EBF2; --background-table-head: #F0F2F5;
        --background-primary: #3D5376; --background-primary-hover: #31425E;
        --background-surface-accent: #E6EBF2; --background-surface-accent-hover: #D7DDE6;
        --stroke-primary: #8190A8; --stroke-secondary: #A4AFC1; --stroke-error: #B35D40;
        --icon-success: #00D400; --icon-warning: #FFCC00;
        --radius-pill: 1000px; --radius-xs: 8px; --radius-s: 16px; --radius-m: 32px;
    ...
    * { box-sizing: border-box; }
    ...
    body {
        ...
        background: var(--background-section-light); color: var(--text-main);
    ...
    .nav {
        ...
        background: var(--background-page); border-radius: var(--radius-pill);
        box-shadow: var(--shadow-l);
    }
    .nav-right { display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
    .brand { display: flex; align-items: center; gap: 10px; color: var(--text-accent); }
    .brand .word { font-weight: 700; font-size: 18px; letter-spacing: .06em; }
    .brand .cross { font-weight: 300; font-size: 18px; color: var(--text-tertiary); }
    ...
    label { display: block; font-size: 14px; font-weight: 500; color: var(--text-secondary); margin: 0 0 8px 22px; }
    input {
        width: 100%; height: 52px; padding: 0 22px; font: inherit;
        background: var(--background-section-light); color: var(--text-primary);
        border: 1px solid transparent; border-radius: var(--radius-pill);
        transition: background-color .2s ease-in-out, border-color .2s ease-in-out;
    }
    input::placeholder { color: var(--text-tertiary); }
    input:hover { background: var(--background-surface-accent); }
    input:focus {
        outline: none; background: var(--background-page);
        border-color: var(--stroke-primary); box-shadow: 0 0 0 3px rgba(129,144,168,.18);
    }
    button {
    ...
    button.secondary { background: var(--background-surface-accent); color: var(--text-primary); }
    button.secondary:hover { background: var(--background-surface-accent-hover); }
    button.danger { background: transparent; color: var(--text-error); box-shadow: inset 0 0 0 1px rgba(179,93,64,.5); }
    button.danger:hover { background: rgba(179,93,64,.10); }
    button.small { height: 40px; padding: 0 20px; font-size: 14px; }
    button.link {
        height: auto; padding: 0; background: none; color: var(--text-secondary);
        font-size: 15px; text-decoration: underline;
    }
    button.link:hover { background: none; color: var(--text-accent); }
    .btn-pill {
        display: inline-flex; align-items: center; height: 44px; padding: 0 24px;
        border-radius: var(--radius-pill); background: var(--background-primary);
        color: var(--text-main-alt); font-weight: 500; transition: background-color .3s ease-in-out;
    }
    .btn-pill:hover { background: var(--background-primary-hover); text-decoration: none; }
    .nav-link {
        display: inline-flex; align-items: center; height: 44px; padding: 0 18px;
        border-radius: var(--radius-pill); color: var(--text-secondary); font-weight: 500;
    ...
    th {
        ...
        padding: 10px 16px; background: var(--background-table-head); white-space: nowrap;
    }
    th:first-child { border-radius: var(--radius-xs) 0 0 var(--radius-xs); }
    th:last-child { border-radius: 0 var(--radius-xs) var(--radius-xs) 0; }
    td { padding: 14px 16px; border-bottom: 1px solid var(--background-section-dark); vertical-align: middle; }
